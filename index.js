...
function makeSmartPerson(name) {
  return {
    ...
    speak: function() { // Not an arrow due to this
...
function getCarInfoByIndex(inventory, index) {
  const {car_make: make, car_model: model} = inventory[index];
  return `This is a car ${make} ${model}`;
}
...
function getCarInfoById(inventory, id) {
  for (let i=0; i<inventory.length; i++) if (inventory[i].id===id) return getCarInfoByIndex(inventory, i);
}
...
function sortCarInventory(inventory) {
  return inventory.sort(({car_model: model1}, {car_model: model2}) => model1 > model2 ? 1 : model2 > model1 ? -1 : 0);
}
...
function getModelYears(inventory) {
  const years = [];
  for (const {car_year: year} of inventory) years.push(year);
  return years;
}
...
function getOlderCars(inventory, maxYear) {
  const oldCars = [];
  for (const car of inventory) if (car.car_year <= maxYear) oldCars.push(car);
  return oldCars;
}
...
function getGermanCars(inventory) {
  ...
  for (const car of inventory) {
    if (car.car_make === 'Audi' || // array.contains and array.indexOf for cleaner code disallowed
...
function carMaker(odometer) {
  return {
    odometer: odometer,
    drive: function(distance) { // Not an arrow due to this
